...
test("launch agent starts the watcher without embedding its secret", () => {
  ...
  assert.deepEqual(paths, {
    supportDir: join(home, "Library/Application Support/LixiReviewDocsSync"),
    runtimeDir: join(home, "Library/Application Support/LixiReviewDocsSync/runtime"),
    configPath: join(home, "Library/Application Support/LixiReviewDocsSync/config.json"),
    logsDir: join(home, "Library/Logs/LixiReviewDocsSync"),
    stdoutPath: join(home, "Library/Logs/LixiReviewDocsSync/stdout.log"),
    stderrPath: join(home, "Library/Logs/LixiReviewDocsSync/stderr.log"),
    plistPath: join(
      home,
      "Library/LaunchAgents/com.wendell.lixi-review-docs-realtime-sync.plist",
    ),
  });
  ...
  assert.match(plist, /<key>Label<\/key>\s*<string>com\.wendell\.lixi-review-docs-realtime-sync<\/string>/);
