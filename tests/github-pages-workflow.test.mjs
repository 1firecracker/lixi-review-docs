...
test("GitHub Pages build targets the independent lixi-review-docs path", async () => {
  const config = await readFile(new URL("../vite.pages.config.ts", import.meta.url), "utf8");

  assert.match(config, /base:\s*["']\/lixi-review-docs\/["']/);
  assert.doesNotMatch(config, /project-radar-docs/);
});
