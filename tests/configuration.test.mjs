...
test("uses independent LIXI defaults and identifiers", async () => {
  const paths = [
    "app/layout.tsx",
    "github-pages/index.html",
    "lib/site-config.ts",
    "scripts/snapshot-docs.mjs",
    "sync/install-launch-agent.mjs",
  ];
  const sources = await Promise.all(paths.map((path) => readFile(join(root, path), "utf8")));
  const operationalSource = sources.join("\n");

  assert.doesNotMatch(operationalSource, /Project Radar|project-radar-docs|baowenzhuo/);
  assert.match(operationalSource, /LIXI Review Docs/);
});
